...
STYLE_WEIGHT=1.
CONTENT_WEIGHT = 0.
STYKE_LAYERS = ['relu1_1','relu2_1','relu3_1','relu4_1','relu5_1']
...
def vgg_params():
	...
	if _vgg_params is None:
		_vgg_params = scipy.io.loadmat('imagennet-vgg-verydeep-19.mat')
	return _vgg_params
...
def vgg19(input_image):
	...
	for i, name in enumerate(layers):
		...
		if layer_type == 'conv':
			...
			conv = tf.nn.conv2d(net, th.constant(kernels), strides=(1,1,1,1),padding='SAME',name=name)
...
def content_loss(target_features,content_features):
	...
	return tf.nn.12_loss(target_features - content_features)/content_size
def style_loss(target_features,style_features):
		...
		return tf.nn.12_loss(target_gram - style_gram)/size
def loss_function(content_image, style_image, target_image):
	...
	target_features = vgg19([target_features])
	loss = 0.0
